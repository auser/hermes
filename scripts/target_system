...
%%! -pa ./ebin -pz ./deps/stoplight/ebin 
...
do_main(install, [File, InstallTo]) ->
    install(File, InstallTo);
do_main(create, RelName) ->
    create(RelName).
...
%% RelFileName the *stem* without trailing .rel, .script etc.
%% create(RelFileName)
create(RelFileName) ->
